        ...
        <section className="bg-zinc-950 py-24 px-6 border-b border-white/10 relative overflow-hidden">
            ...
            <div className="max-w-[1400px] mx-auto mb-12 flex flex-col md:flex-row justify-between items-end gap-6 relative z-10">
                ...
                <div className="text-right hidden md:block">
                    ...
                    <div className="text-xs text-white/40 uppercase tracking-widest font-bold">Based on Google Reviews</div>
                </div>
            </div>

            {/* Scrolling Review Cards Container */}
            <div className="relative w-full overflow-hidden group">
                ...
                {/* Marquee Track */}
                <div
                    className="flex gap-6 animate-scroll hover:animation-play-state-paused w-max"
                >
                    {/* Triple the reviews to ensure smooth infinite scroll on wide screens */}
                    {[...reviews, ...reviews, ...reviews].map((review, idx) => (
                        <div
                            key={`${review.author_name}-${idx}`}
                            className="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 w-[350px] md:w-[400px] shrink-0 hover:border-amber-500/30 hover:bg-amber-500/5 transition-all duration-500 group/card select-none cursor-default rounded-sm"
                        >
